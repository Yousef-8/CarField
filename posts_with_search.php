...
<header>
    ...
    <nav>
        ...
        if (isset($_SESSION['username'])) {
            echo '<a href="posts_with_search.php">Posts</a>';
            echo '<a href="user/update_profile.php">Profile</a>';
            echo '<a href="user/upload_posts.php">Upload-posts</a>';
            echo '<a href="logout.php">Logout</a>';
        } else {
            echo '<a href="user/login.php">Signin</a>';
          echo  '<a href="user/register.php">Register</a>';
          echo  '<a href="admin/admin_login.php">Admin</a>';
        }
        ?>
...
<div class="post-container">
    <?php foreach ($posts as $post) { ?>
        ...
        <div class="profile">    
                ...
                <div class="uploader">
                 <h1> <strong> <?php echo  $post["username"]; ?></strong> </h1>
                </div>
            ...
            <?php
            $postId = $post['post_id'];
            $commentsSql = "SELECT * FROM comments WHERE comment_post_id = ? ORDER BY comment_id ASC";
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['delete_comment'])) {
    ...
    $checkOwnershipStmt = $connection->prepare($checkOwnershipSql);
    ...
    if ($_SESSION['username'] === $commentOwner) {
        ...
        $deleteCommentStmt = $connection->prepare($deleteCommentSql);
...
//Asynchronous JavaScript and XML (AJAX) = a set of web development techniques that allows web pages to be updated asynchronously 
    
$(document).ready(function() { //this function only executes when html content is loaded
        ...
        $('#keyword').keyup(function() {
            ...
            $.ajax({
                ...
                data: { keyword: keyword }, //:keyword which is the search i gave above
                success: function(response) { //this is a callback func that is executed if request is successful
